...
namespace MyProjectUI.Areas.XiTong.Controllers
{
    public class InspeTaskmController : Controller
    {
        ...
        public ActionResult SelectData()
        {
            ...
            string sql = "select * from [ps_inspectiontask_main] where 1=1 and isCancel=0 ";
            ...
            while (dr.Read())
            {
                ...
                main.issuedByCode = dr["issuedByCode"].ToString();
                main.issuedByName= dr["issuedByName"].ToString();
                main.issuedTime= Convert.ToDateTime(dr["issuedTime"].ToString());
        ...
        public ActionResult UpdTaskData()
        {
            ...
            var users = (from u in medc.ps_user
                ...
                }else
                {
                        ...
                        var user = (from c in medc.ps_user
                                    where c.userCode == inspectorcode
                                    select c).FirstOrDefault();
                        ps_inspector_detail detail = new ps_inspector_detail()
                        {
                            taskId = id,
