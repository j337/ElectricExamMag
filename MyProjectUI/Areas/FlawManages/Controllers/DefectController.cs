...
namespace MyProjectUI.Areas.FlawManages.Controllers
{
    public class DefectController : Controller
    {
        ...
        public ActionResult DefectTableDataList()
        {
            ...
            var result1 = (from a in medc.ps_inspectiontask_detail
                           join b in medc.ps_inspectiontask_main on a.taskid equals b.id
                           where a.taskStatus == 4 && a.isBugLevel == 0&&a.isBug==1&&a.isSolve==0&&b.taskStatus==4
            ...
            if (State)
            {
                for (int i = 0; i < str1.Length; i++)
                {
                    ...
                    try
                    {
                        ...
                        result.isBugLevel = 1;
                        var syslist = (from s in medc.ps_systemconfig
                                       where s.configCode == "BUG_LEVEL" && s.configValueId == int.Parse(str2[1])
                                       select s).FirstOrDefault();

                        result.bugLevelName = syslist.configValueName;
